...
test_missing_directory() {
    ...
    assert_err_contains "error message" "no directory provided"
}
...
test_cd_failure() {
    ...
    assert_err_contains "cd error" "Failed to change to directory"
}

test_stash_save_failure() {
    run_script "$TEST_DIR/fail-stash"
    assert_rc "stash save fails" 1
    assert_err_contains "stash error" 'ERROR: `git stash save --include-untracked "backup-2025-01-15-1430.45"` failed'
}

test_checkout_b_failure() {
    run_script "$TEST_DIR/fail-checkout"
    assert_rc "checkout -b fails" 1
    assert_err_contains "checkout error" 'ERROR: `git checkout -b "backup-2025-01-15-1430.45"` failed'
}

test_stash_apply_failure() {
    run_script "$TEST_DIR/fail-apply"
    assert_rc "stash apply fails" 1
    assert_err_contains "apply error" 'ERROR: `git stash apply` failed'
}

test_add_failure() {
    run_script "$TEST_DIR/fail-add"
    assert_rc "git add fails" 1
    assert_err_contains "add error" 'ERROR: `git add .` failed'
}

test_commit_failure() {
    run_script "$TEST_DIR/fail-commit"
    assert_rc "git commit fails" 1
    assert_err_contains "commit error" 'ERROR: `git commit -m "Backup 2025-01-15-1430.45"` failed'
}

test_push_failure() {
    run_script "$TEST_DIR/fail-push"
    assert_rc "git push fails" 1
    assert_err_contains "push error" 'ERROR: `git push "origin" "backup-2025-01-15-1430.45"` failed'
}

test_checkout_dash_failure() {
    run_script "$TEST_DIR/fail-checkout-dash"
    assert_rc "checkout - fails" 1
    assert_err_contains "checkout - error" 'ERROR: `git checkout -` failed'
}

test_branch_delete_failure() {
    run_script "$TEST_DIR/fail-branch"
    assert_rc "branch -D fails" 1
    assert_err_contains "branch error" 'ERROR: `git branch -D "backup-2025-01-15-1430.45"` failed'
}
...
test_stash_apply_conflict_path() {
    ...
    assert_err_contains "apply failed" 'ERROR: `git stash apply` failed'
}
...
SHIM
    ...
    assert_err_contains "missing remote error" "remote 'origin' not configured"
