...
UNDER_TEST="$SCRIPT_DIR/../scripts/httpcode"
...
test_missing_jq() {
    ...
    assert_stderr_contains "missing jq error" "[ERR][httpcode] jq is required"
    assert_stderr_contains "missing jq suggest install" "Homebrew"
}
...
test_invalid_status_code() {
    ...
    assert_stderr_contains "999 error message" "[ERR][httpcode] Unknown or unsupported HTTP status code"
}
...
test_no_arguments() {
    ...
    assert_stderr_contains "no args error message" "[ERR][httpcode] Status code is required"
}
