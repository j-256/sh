...
UNDER_TEST="$SCRIPT_DIR/../scripts/screenshot-rename"
...
test_unknown_option() {
    ...
    assert_stderr_contains "unknown msg" "[ERR][screenshot-rename] Unknown argument '--unknown'. Run \`screenshot-rename -h\` for usage"
}

test_unknown_short() {
    run_script -x
    assert_rc "unknown short exits 2" 2
    assert_stderr_contains "unknown short msg" "[ERR][screenshot-rename] Unknown argument '-x'. Run \`screenshot-rename -h\` for usage"
}
...
test_fswatch_missing() {
    ...
    assert_stderr_contains "no fswatch msg" "[ERR][screenshot-rename] fswatch is required"
}
...
test_fswatch_after_utc() {
    ...
    assert_stderr_contains "no fswatch utc msg" "[ERR][screenshot-rename] fswatch is required"
}
