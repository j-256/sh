...
test_documentation_covers_help_surface() {
    ...
    header_environment="$(printf '%s\n' "$header" | grep -Eo '(NOTIFY_[A-Z_]+|NO_COLOR|CLICOLOR_FORCE)' | sort -u)"
    help_environment="$(grep -Eo '(NOTIFY_[A-Z_]+|NO_COLOR|CLICOLOR_FORCE)' "$TEST_DIR/stdout" | sort -u)"
    doc_environment="$(grep -Eo '(NOTIFY_[A-Z_]+|NO_COLOR|CLICOLOR_FORCE)' "$DOC_UNDER_TEST" | sort -u)"
