...
UNDER_TEST="$SCRIPT_DIR/../scripts/tsd"
...
test_no_input() {
    ...
    assert_stderr_contains "no input error" "[ERR][tsd] No input provided. Run \`tsd -h\` for usage"
}

test_multiple_positional() {
    run_script 1234 5678
    assert_rc "multiple positional exits 2" 2
    assert_stderr_contains "multiple positional error" "[ERR][tsd] Unknown argument '5678'. Run \`tsd -h\` for usage"
}

test_multiple_units() {
    run_script 1234 -s -m
    assert_rc "multiple units exits 2" 2
    assert_stderr_contains "multiple units error" "[ERR][tsd] Multiple units not allowed"
}

test_unknown_option() {
    run_script 1234 --invalid
    assert_rc "unknown option exits 2" 2
    assert_stderr_contains "unknown option error" "[ERR][tsd] Unknown argument '--invalid'. Run \`tsd -h\` for usage"
}
