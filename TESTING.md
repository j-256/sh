...
| `meta-surface-parity.test.sh` | Every option the parser accepts is documented in `-h`, and every short option has a long form (CONVENTIONS "Self-sufficiency"). Uses the shared `_option_flags`/`_option_pairs` extractors; skips arms marked `# meta:hidden`, whole `case ... in` blocks marked `# meta:not-options` (a recognizer that matches another tool's flags, e.g. a curl wrapper), and scripts in `$_META_OPT_EXCLUDE` plus its own tracked exclusions |
| `meta-canonical-letters.test.sh` | The behavior-scoped canonical-letter binding (CONVENTIONS "Canonical short options"): a script whose option set contains `--force` also has `-f`, and `--dry-run` also has `-n`. Uses the shared `_option_flags`; waives a long whose arm is marked `# meta:canonical-exempt` (an authored long-only exception, e.g. pin-dns's `--dry-run` whose `-n` is reserved for curl's `--netrc`); skips scripts in `$_META_OPT_EXCLUDE` |
| `meta-diagnostic-format.test.sh` | Every diagnostic helper (`_error`/`_warn`/`_info`/`_debug`) uses the canonical severity-led, bracket-adjacent prefix `[SEV][$SCRIPT_NAME]` (CONVENTIONS "Error Messages"). Positive check: asserts the `[SEV][` token is present in each helper's brace-balanced body (or a `PFX_<SEV>` reference for the structured colored variant, e.g. spf), so legacy shapes (`[SEV] name:`, `[name] SEV:`) fail regardless of form. Carries a synthetic self-check so the detector can't rot to a no-op. Routing (stdout vs stderr) is review-only, not checked |
| `meta-flag-spelling.test.sh` | Options use canonical spellings, not retired legacy ones (CONVENTIONS "Canonical short options"): the dry-run long flag is `--dry-run`, never `--dryrun`. Closes the gap `meta-canonical-letters` leaves (which keys off the literal `--dry-run` and so never sees `--dryrun`). Uses the shared `_option_flags` extractor, so it flags only a real parsed option; banned set is a `bad=good` table seeded with `--dryrun`. Carries a synthetic self-check |
| `meta-dry-run-format.test.sh` | Every script with `--dry-run` defines the canonical `[DRY][$SCRIPT_NAME]` stdout helper and routes counterfactual action headers through it (CONVENTIONS "Dry-run output"). Rejects retired direct emitters such as `[DRY RUN]` and bare `Would` output, and carries synthetic self-checks for helper presence, routing, and direct-emitter detection |
| `meta-underline-style.test.sh` | Every raw stdout underline starts from an empty variable, uses the canonical TTY/`CLICOLOR_FORCE`/`NO_COLOR` gate, and has a paired reset (CONVENTIONS "--help"). Rejects `tput` underline dependencies and carries synthetic self-checks for the legacy gates and canonical palette shapes; per-script tests probe the environment precedence at runtime |
