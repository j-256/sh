...
test_decimal_rejected() {
    printf '%s\n' 1.5 2.5 | run_script
    assert_rc "decimal rejected with rc 1" 1
    assert_err_contains "decimal error message" "Error: Integers only."
}

test_negative_sign_rejected() {
    printf '%s\n' -5 10 | run_script
    assert_rc "negative rejected with rc 1" 1
    assert_err_contains "negative error message" "Error: Integers only."
}

test_letters_rejected() {
    printf '%s\n' abc 123 | run_script
    assert_rc "letters rejected with rc 1" 1
    assert_err_contains "letters error message" "Error: Integers only."
}

test_mixed_valid_invalid() {
    printf '%s\n' 1 2 three 4 | run_script
    assert_rc "mixed rejected with rc 1" 1
    assert_err_contains "mixed error message" "Error: Integers only."
}
