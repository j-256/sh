...
test_missing_start_pattern() {
    ...
    assert_rc "missing start" 1
    assert_err_contains "error message" "[ERR] Start pattern is required."
}

test_missing_start_value() {
    write_test_file "test.txt"
    run_script -s
    assert_rc "missing -s value" 1
    assert_err_contains "error message" "[ERR] Missing value for '-s'."
}

test_missing_end_value() {
    write_test_file "test.txt"
    run_script -s "START" -e
    assert_rc "missing -e value" 1
    assert_err_contains "error message" "[ERR] Missing value for '-e'."
}

test_missing_trim_first_value() {
    write_test_file "test.txt"
    run_script -s "START" -f
    assert_rc "missing -f value" 1
    assert_err_contains "error message" "[ERR] Missing value for '-f'."
}

test_missing_trim_last_value() {
    write_test_file "test.txt"
    run_script -s "START" -l
    assert_rc "missing -l value" 1
    assert_err_contains "error message" "[ERR] Missing value for '-l'."
}

test_missing_trim_value() {
    write_test_file "test.txt"
    run_script -s "START" -t
    assert_rc "missing -t value" 1
    assert_err_contains "error message" "[ERR] Missing value for '-t'."
}
...
test_unknown_option() {
    ...
    assert_err_contains "error message" "[ERR] Unknown option '--unknown-flag'."
}
